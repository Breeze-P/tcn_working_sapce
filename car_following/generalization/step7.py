# 将处理完的数据按网络模型切分为x和y,其中x为(?, timestep, 8),y为(?, 2)
import pandas as pd

data = pd.read_csv("HighD_1_fin.csv", header=0)
...
outputX.to_csv('dataX_highD.csv', index=False)
outputY.to_csv('dataY_highD.csv', index=False)
