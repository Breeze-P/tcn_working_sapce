...
def drawChart(model, x_data, y_data, range_val):
    ...
    graph_up.set_title("GRU us-101 x-axis")  # 标题
    graph_up.legend()  # 图例
    # 画y轴
    ...
    graph_down.set_title("GRU us-101 y-axis")
    graph_down.legend()
    plt.tight_layout()
    plt.show()
...
predict_y = gru_model.predict(test_data_x)
ax.set_title("GRU")
...
bx.legend()
plt.show()
gru_model.save('gru.hdf5')  # 保存模型

# 只画图的话把上面代码注释
gru_model = tf.keras.models.load_model('gru.hdf5')  # 加载模型
drawChart(gru_model, data_test_x, data_test_y, 200)
